...
export const DeepReadView: React.FC<DeepReadViewProps> = ({ paperId, paperMetadata, onBack }) => {
    ...
    // Project picker state for upgrade flow
    const [showProjectPicker, setShowProjectPicker] = useState(false);

    // Adapt sendMessage to match onSend signature () => void
    const handleSend = () => {
        sendMessage();
    };
    ...
    return (
        <>
            <div className="h-[calc(100vh-4rem)] flex overflow-hidden bg-[#0A0A0A]">
                ...
                >
                    {/* Top Toolbar */}
                    <div className="h-12 border-b border-white/10 flex items-center px-4 justify-between bg-[#0A0A0A] sticky top-0 z-10">
                ...
                >
                    {/* Tab Header */}
                    <div className="flex items-center border-b border-white/10 bg-[#0A0A0A] sticky top-0 z-10">
                        <button
                    ...
                    <div className="flex-1 overflow-y-auto relative flex flex-col">
                        {activeTab === 'chat' ? (
                            <>
                                {/* Upgrade Banner for abstract mode */}
                                {chatMode === 'abstract' && (
                                    <UpgradeBanner
                                        onAddToProject={() => setShowProjectPicker(true)}
                                    />
                                )}
            ...
            {/* Project Picker Modal for upgrade flow */}
            <ProjectPickerModal
                isOpen={showProjectPicker}
                onClose={() => setShowProjectPicker(false)}
                paperId={paperId}
                paperTitle={paperMetadata?.title}
                paperSummary={paperMetadata?.summary || paperMetadata?.abstract}
                paperAuthors={paperMetadata?.authors}
                paperUrl={paperMetadata?.url}
                paperPublishedDate={paperMetadata?.published_date}
                paperThumbnail={paperMetadata?.thumbnail}
                onIngestionComplete={() => {
                    // Reset chat to trigger re-detection of ingestion status
                    startNewChat();
                }}
            />
        </>
